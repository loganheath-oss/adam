...
TASKS: list[dict] = [
    ...
    {"surface": "chat", "task": "Log an operator issue",
     "level": "ACTION", "source": "tool_log_issue → db.report_issue (visible in admin + Digest; near-duplicate guard)"},
    {"surface": "chat", "task": "Save the operator's Gate-3 concept selection",
     "level": "ACTION", "source": "tool_select_copy_concepts (state-gated to awaiting_gate_3; ≥1 selected enforced; JSON+CSV kept in sync)"},
    {"surface": "chat", "task": "Apply the operator's Gate-3 copy edits",
     "level": "ACTION", "source": "tool_edit_copy (state-gated to awaiting_gate_3; whitelisted fields, type/length-validated; provenance note on the concept)"},
