...
|------|------|--------------------|--------------|
| 2 | Order + Refs | `get_sprint` + `get_references` | The order summary (driver, platform, format, quantity, styles, audience, due date) AND the reference context (how many refs were loaded, brand voice, targeting examples). Frame it: "Before we spend any AI credits, let's make sure I got the order right and loaded the right references." **If the order has NO brief, lead with a prominent warning: "⚠️ This order has no brief — copy will follow the standing reference docs only, with no sprint theme or key messaging. Want to add one now?" and offer `edit_order` to add it before gate 2 is approved.** **If the user asks to change the brief, quantity, delivery date, driver, or targeting at this gate, call `edit_order` to apply the change, then re-show the updated summary. Do NOT tell them you can't change it — you can.** |
