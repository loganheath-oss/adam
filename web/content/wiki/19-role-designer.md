...
Templates`, `-> Reddit Templates`, and so on) → the Template Library page → the whole file. The
platform step matters: several pages hold containers with identical names, so this is what stops a
Meta run pulling a LinkedIn frame.
...
exact. Cosmetic drift, such as a trailing slash, doubled spaces, or different capitalization, is
now tolerated and reported rather than silently breaking a style, but it should still be cleaned
up.
